...
export default function ComercioDetails() {
  ...
  useEffect(() => {
    ...
    // Buscar detalhes do comércio pelo ID/slug na URL
    const companyId = params.id as string;

    if (companyId) {
      // Encontrar o comércio nos dados mockados
      // Na implementação real, isso seria uma chamada de API
      const foundCompany = mockCompanys.find(
        (comp) =>
          // Verificar pelo ID (se existir) ou pelo slug do nome
          (comp.id && comp.id.toString() === companyId) ||
          normalizeText(comp.name) === companyId
      );

      if (foundCompany) {
        // Adicionar dados fictícios extras para a página de detalhes
        setCompany({
          ...foundCompany,
          phone: "(48) 00000-0000",
          description: "Pub e Trucks, localizado atrás do supermercado Rosa.",
          hours: "Todos os dias das 18h à 0h",
          address:
            "Avenida Prefeito Nelson Martins, Centro - Palhoça/SC - 88131-300",
          location: {
            lat: -27.64662,
            lng: -48.667361,
          },
    ...
    // Finalizar carregamento após um pequeno delay (simulando API)
    setTimeout(() => {
      setIsLoading(false);
    }, 500);
  }, [params.id]);
        ...
        <div className="max-w-[1272px] mx-auto px-7 py-5">
          <div className="w-full py-12 text-center bg-red-50 rounded-lg border border-red-200 px-4">
            ...
            <p className="text-gray-600 max-w-md mx-auto mb-4">
              Não foi possível encontrar o comércio solicitado.
            </p>
            <Button
              variant="default"
